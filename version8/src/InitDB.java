    ...
    public static void main(String[] args) {
        ...
        try (Connection conn = DriverManager.getConnection(healthUrl, DB_USER, DB_PASS)) {
            ...
            execUpdate(conn,
                ...
                "  waist DECIMAL(5,2) CHECK (waist > 0 AND waist < 300)," +
            ...
            // 兼容旧库：补体重/腰围（必需属性根数据）
            execUpdate(conn, "ALTER TABLE users ADD COLUMN IF NOT EXISTS weight DECIMAL(5,2) CHECK (weight > 0 AND weight < 500)");
            execUpdate(conn, "ALTER TABLE users ADD COLUMN IF NOT EXISTS waist DECIMAL(5,2) CHECK (waist > 0 AND waist < 300)");
            ...
            execUpdate(conn,
                ...
                "  subcutaneous_fat DECIMAL(4,2) CHECK (subcutaneous_fat >= 0 AND subcutaneous_fat <= 100)," +
            ...
            // 兼容旧表：补仪器测量字段
            execUpdate(conn, "ALTER TABLE health_records ADD COLUMN IF NOT EXISTS protein_rate DECIMAL(4,2) CHECK (protein_rate >= 0 AND protein_rate <= 100)");
            execUpdate(conn, "ALTER TABLE health_records ADD COLUMN IF NOT EXISTS subcutaneous_fat DECIMAL(4,2) CHECK (subcutaneous_fat >= 0 AND subcutaneous_fat <= 100)");
            execUpdate(conn, "ALTER TABLE health_records ADD COLUMN IF NOT EXISTS body_score INT");
