...
/**
 * AI 问答记录面板（JavaFX 8 重写 version1 AISystemPanel 问答部分）
 * 列表、双击查看、标记有效/无效（无效时给记录用户发审核通知）。
 * getAIChatRecords() 返回 String[]{id,username,question,status,time}。
 */
public class AIChatRecordPanel extends VBox {
    ...
    private void load() {
        ...
        for (String[] row : DBUtil.getAIChatRecords()) {
    ...
    private void viewRecord(int id) {
        Map<String, String> rec = DBUtil.getAIChatRecordById(id);
        if (rec == null || rec.isEmpty()) {
            warn("未找到该记录");
            return;
        }
        TextArea ta = new TextArea(rec.getOrDefault("answer", ""));
        ...
        a.setTitle("AI 回答详情 - " + rec.getOrDefault("username", "") + " [" + rec.getOrDefault("status", "") + "]");
    ...
    private void updateStatus(String status) {
        ...
        if (id < 0) return;
        if (DBUtil.updateAIChatStatus(id, status)) {
            if ("无效".equals(status)) notifyRejected(id);
            DBUtil.logAction("ADMIN", DBUtil.currentUsername, "审核AI问答", "ID=" + id + ", 状态=" + status);
            load();
        } else {
            warn("操作失败（可能已是该状态）");
        }
    ...
    private void notifyRejected(int id) {
        ...
        if (username != null && !username.isEmpty()) {
            DBUtil.saveNotification("系统", username, "AI 内容审核通知",
                    "您的一条 AI 问答记录（ID=" + id + "）被管理员判定为无效，已对您隐藏。" +
